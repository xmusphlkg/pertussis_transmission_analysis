...
p2a <- ggplot(scenario_params, aes(parameter, scenario_label, fill = value)) +
  ...
  scale_fill_gradient(low = "#F0F0F0", high = "#2166AC",
                      limits = c(0, 0.9), labels = percent_format(accuracy = 1)) +
...
# --- Panel C: Infection Reduction vs Infant-Case Reduction Trade-off ---
p2c <- vaccine_summary %>%
  filter(scenario != "no_vaccine") %>%
...
# --- Panel D: Infection-Source Decomposition (stacked bar) ---
...
p2d <- ggplot(source_data, aes(share, scenario_label, fill = origin)) +
...
figure2 <- ((p2a | p2b) / (p2c | p2d)) +
  plot_layout(heights = c(1, 1), widths = c(0.8, 1.2)) +
  plot_annotation(tag_levels = "A") &
  theme(plot.tag = element_text(face = "bold", size = 8.5))

save_main_figure(figure2, "figure_2_vaccine_mechanisms", height = 8.0)
cat("Figure 2 saved.\n")
