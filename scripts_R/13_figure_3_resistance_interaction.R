...
# Get starting resistance for annotation
start_resistance <- resistance_ts %>%
  filter(year == 0) %>%
  select(country_label, resistant_fraction) %>%
  rename(start_frac = resistant_fraction)
...
# ============================================================================
fitness_ts <- read_model_table(model_path("outputs", "simulations", "resistance_fitness_sensitivity")) %>%
  mutate(
    country_label = factor(format_country(country), levels = country_label_levels),
    year = time / 365,
...
# Aggregate across countries (median + IQR)
fitness_ts_agg <- fitness_ts %>%
  filter(year <= 5) %>%
  group_by(year, fitness_label) %>%
  summarise(
    median_frac = median(resistant_fraction, na.rm = TRUE),
    q25 = quantile(resistant_fraction, 0.25, na.rm = TRUE),
    q75 = quantile(resistant_fraction, 0.75, na.rm = TRUE),
    .groups = "drop"
  )
...
p3b <- ggplot(fitness_ts_agg, aes(year, median_frac, colour = fitness_label, fill = fitness_label)) +
  ...
  labs(x = "Year", y = "Resistant fraction (median ± IQR)",
...
if (nrow(fitness_summary) > 0) {
  ...
  label_data_d <- fitness_surface %>%
    mutate(label = scales::percent(median_resistant_end, accuracy = 1)) %>%
    filter(grid_fitness_R %in% c(0.7, 1.0, 1.25),
           grid_VE_inf %in% c(0.05, 0.25, 0.55))
...
if (nrow(fitness_summary) > 0) {
  label_data_e <- fitness_surface %>%
    mutate(label = format(round(median_infant, 0), big.mark = ",")) %>%
    filter(grid_fitness_R %in% c(0.7, 1.0, 1.25),
           grid_VE_inf %in% c(0.05, 0.25, 0.55))
