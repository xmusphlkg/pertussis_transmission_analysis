...
if (nrow(country_sim) > 0) {
  ...
  p_ed6a <- baseline_snapshot %>%
    ...
    labs(x = "All infections per 100,000/year (log)", y = NULL) +
    theme_nature()

  p_ed6b <- baseline_snapshot %>%
    ggplot(aes(annualized_infant_cases_per_100k, country_burden_order)) +
    geom_point(size = 1.8, colour = "#009E73") +
    scale_x_log10(labels = label_number(accuracy = 1)) +
    labs(x = "Infant cases per 100,000 infants/year (log)", y = NULL) +
    theme_nature()
  ...
  p_ed6c <- baseline_snapshot %>%
    ...
    geom_segment(aes(x = resistant_fraction_start, xend = resistant_fraction_end, yend = country_burden_order),
                 linewidth = 0.45, colour = "#D55E00", alpha = 0.75) +
    geom_point(aes(x = resistant_fraction_start), shape = 21, fill = "white", colour = "#D55E00", size = 1.5, stroke = 0.3) +
    geom_point(aes(x = resistant_fraction_end), colour = "#D55E00", size = 1.8) +
