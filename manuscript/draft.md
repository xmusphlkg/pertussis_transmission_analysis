...
The model tracked 8 age groups (0–2 months, 3–11 months, 1–4 years, 5–9 years, 10–17 years, 18–39 years, 40–64 years, and ≥65 years), chosen to separate the maternal protection window, primary immunization series, school-age booster period, adolescent waning period, reproductive-age adults (primary infant contact source), and elderly populations with distinct clinical presentation. Infections were divided into macrolide-sensitive and macrolide-resistant strains. Susceptible individuals retained origin histories (unvaccinated, maternally protected, 1-dose recent/waned, 2-dose recent/waned, 3-or-more-dose recent/waned), so that four vaccine mechanism parameters—VE_sus (reduced susceptibility to infection), VE_sym (reduced symptomatic disease), VE_inf (reduced onward infectiousness), and VE_dur (reduced infectious duration)—could act on infection source history rather than on a single aggregate vaccinated state. The resulting state space comprised 74 compartments per age group (592 total state variables).

Immunity dynamics followed a SIRWS (Susceptible-Infected-Recovered-Waned-Susceptible) structure with immune boosting (Lavine et al., 2011; Wearing & Rohani, 2009). After recovery, individuals entered a fully immune state (R) that waned over approximately 5 years to a partially immune "waned but boostable" state (W). Individuals in W who were re-exposed to circulating pertussis had their immunity restored to R without developing symptomatic infection (boosting efficiency = 0.70). Those in W who were not boosted eventually lost all immunity and returned to S over approximately 10 years. This mechanism naturally produces immunity debt during periods of reduced pathogen circulation: when COVID-19 non-pharmaceutical interventions suppressed pertussis transmission, the absence of natural boosting caused W to accumulate and eventually transition to S, creating a pool of newly susceptible individuals that fueled post-pandemic surges upon NPI relaxation.

Transmission was driven by country-specific contact matrices with time-varying COVID-19 NPI contact reductions, annual seasonality, demographic aging and birth turnover, routine vaccination maintenance, low-level importation, and strain-specific treatment effects. The force of infection incorporated postexposure prophylaxis as a prevalence-triggered modifier with strain-specific effectiveness, so that macrolide resistance directly reduced PEP benefit without requiring a separate prophylaxis compartment.
