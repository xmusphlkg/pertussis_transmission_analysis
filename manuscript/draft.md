...
**Main text word count:** 2976

## Key Points

**Question:** Which vaccination, exposure-reduction, and resistance-management profiles optimized modeled infant pertussis outcomes under predefined decision constraints?
...
Dominance was assessed within each country and constraint: a strategy was dominated if another allowed strategy had at least as large an infant-case reduction, at least as large a resistant-infection reduction, and no greater implementation-intensity score, with improvement in at least 1 dimension. Preferred profiles minimized infant cases under each constraint; non-dominated sets defined decision frontiers rather than a single recommendation. Implementation intensity was not interpreted as cost; it was an ordinal feasibility proxy used only in dominance and visualization.
...
In preference-threshold analyses, resistance-guided management was preferred at resistant-infection weight 0.21 or lower in 6 profiles, required 0.97 or higher in 3, and was never preferred in the United States, where baseline resistant infections were zero (<span style="color:#5DADE2;">Figure 4C</span> and <span style="color:#5DADE2;">eFigure 10D</span>). With at least 50% resistant-infection reduction required, resistance-guided management was preferred in 6 profiles, with median standardized infant-case regret of 8.1%.

The supplementary program-portfolio factorial ranked a layered timeliness, infant-exposure, targeted PEP, and resistance-management package lowest for modeled infant cases in all 10 profiles, with a median 83% infant-case reduction (IQR, 65%-94%) (<span style="color:#5DADE2;">eFigure 11A</span>). This was interpreted as an implementation stress test, not a primary deployable policy recommendation.

### Future Product-Target Mechanism Analysis

Future product-target analyses favored profiles with stronger transmission-blocking assumptions rather than symptom protection alone (<span style="color:#5DADE2;">Figure 4A and 4D</span>). Within this hypothetical domain, the combined future stress-test profile was lowest in 8 profiles and the high-transmission-blocking vaccine target in 2. Across illustrative profiles, the combined and high-transmission-blocking profiles had median infant-case reductions of 99% and 97%, respectively. The transmission-blocking-only target had a median 95% infant-case reduction but was dominated by profiles with stronger combined infection, transmission, or management effects in the outcome frontier.
...
A supplementary portfolio factorial supported portfolio thinking for local planning, but the highest-performing layered package had the greatest implementation intensity; primary inferences therefore remain predefined profiles under explicit feasibility constraints.
