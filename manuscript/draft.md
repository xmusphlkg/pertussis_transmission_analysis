# Infant Pertussis Prevention and Macrolide-Resistance Management Across 10 Country Profiles: A Decision Analytical Model
...
**Main text word count:** 2998

## Key Points

**Question:** Which vaccination, exposure-reduction, and resistance-management strategies were favored in a model of infant pertussis prevention across 10 illustrative country profiles?

**Findings:** In this decision analytical model, routine vaccination timeliness and infant-exposure reduction packages most often minimized modeled infant cases under currently implementable program options. Resistance-guided management did not usually minimize all infant cases but became a non-dominated option when resistant infections were considered.

**Meaning:** Modeled infant pertussis prevention priorities depended on implementation constraints, resistance assumptions, and whether decision makers prioritized infant cases alone or also resistant infections.
...
**Objective:** To compare conditional rankings of infant pertussis prevention strategy profiles across vaccination, exposure-reduction, resistance-management, and future transmission-blocking vaccine scenarios.

**Design, Setting, and Participants:** Deterministic age-structured decision analytical transmission model with a 15-year burn-in and 26-year saved horizon beginning January 1, 2025, with 5- and 10-year horizon sensitivities. Ten illustrative, purposively selected country profiles used evidence accessed through May 9, 2026. Analyses used population-level inputs.
...
We used an age-structured decision analytical model to compare conditional infant-protection strategy rankings across vaccination, exposure-reduction, resistance-management, and future product-target scenarios in 10 illustrative country profiles.
...
Because comparable infant incidence or age-specific surveillance targets were unavailable across all profiles, the infant endpoint was used only for within-profile scenario comparison. The primary outcome was relative modeled infant-case change conditional on specified assumptions; absolute modeled infant incidence was not used for burden inference. Thus, the estimand was a conditional ranking under specified age-mixing and vaccine-mechanism assumptions, not expected infant burden or national policy impact. IQRs across illustrative profiles summarize empirical heterogeneity, not uncertainty intervals.
...
Strategy profiles were assigned to decision domains used in the optimization constraints (<span style="color:#5DADE2;">Table 1</span> and <span style="color:#5DADE2;">eTable 4</span>). Program-only options included current practice, nominal coverage floor without timeliness improvement, routine timeliness, adolescent booster scale-up, pregnancy Tdap scale-up, close-contact adult adjuncts, an infant-exposure reduction package, and targeted high-risk PEP. The infant-exposure package combined pregnancy Tdap, close-contact adult protection, reproductive-age adult boosting, and reduced adult-to-infant contact; it was decomposed in supplementary component analyses. Resistance-management options added resistance-guided treatment and resistant-strain PEP assumptions. Future product-target options included infection-blocking, transmission-blocking, high-transmission-blocking vaccine profiles, and combined scenarios.^28,29^
...
Under program-only constraints (<span style="color:#5DADE2;">Figure 4A and 4B</span>), the infant-case-minimizing profile was the infant-exposure reduction package in 5 country profiles, routine timeliness in 4, and adolescent booster scale-up in 1. With implementation intensity included in the dominance rule, infant-exposure reduction was non-dominated in 8 profiles and routine timeliness in 7; current practice remained non-dominated as the lowest-intensity reference but was not infant-case minimizing. The nominal coverage floor without timeliness improvement showed little modeled infant-case change (median change, -1%; IQR, -5% to -1%).
...
Program levers acting through older-age transmission were more heterogeneous. The infant-exposure reduction package yielded 38% lower median modeled reported cases and 35% lower all infections across illustrative profiles.

### Optimization With Resistance Management

Adding resistance-guided management did not change the primary infant-case-minimizing profile in any country: program plus resistance-management optimization still selected the infant-exposure package in 5 profiles, routine timeliness in 4, and adolescent booster scale-up in 1 (<span style="color:#5DADE2;">Figure 4A</span>). However, resistance-guided management became non-dominated in 9 profiles because it substantially reduced resistant infections while not always minimizing all-strain infant cases. Under the specified resistance-guided composite, modeled annualized resistant infections declined from a median of 574 to 2 per 100,000; the median resistant-infection reduction was 94% across all profiles and 97% (IQR, 62%-100%) among 9 nonzero-resistance profiles (<span style="color:#5DADE2;">Figure 4B</span>). This is a mechanistic scenario result conditional on testing uptake, turnaround, alternative antibiotics, adherence, and PEP delivery, not an empirically validated program effect.
...
For health ministries, advisory committees, and surveillance programs, these results are a prioritization screen: strengthen routine timeliness and infant-exposure prevention where feasible, pair MRBP concern with susceptibility or sequencing surveillance, and apply local costs, equity, acceptability, and implementation capacity before choosing programs. Program-only scenarios were closest to implementable decisions. Pregnancy Tdap scale-up yielded modest, consistent modeled infant-case reductions, consistent with maternal vaccination studies.^26,27^ The infant-exposure package showed larger reductions but depended on adult boosting, contact assumptions, and pregnancy Tdap. Targeted high-risk PEP should be read as outbreak or contact-management support.^30,31^ Nominal coverage-floor findings were not evidence against childhood vaccination; they reflected marginal nominal changes without timeliness improvement or stronger transmission blocking.
...
**Data Sharing Statement:** Processed inputs, model code, configuration files, generated outputs, figure source data, and reproducibility files are publicly available without access restrictions at https://github.com/xmusphlkg/pertussis_transmission_analysis under an MIT license and archived in the versioned repository tag `jama-submission-2026-05-29`. Reproduction files include `requirements.txt`, `install_R_packages.R`, `Makefile`, runtime metadata, and figure source-data manifests. A final immutable commit hash and Zenodo DOI should be inserted after repository archival. No individual-level participant data were used or are available.
