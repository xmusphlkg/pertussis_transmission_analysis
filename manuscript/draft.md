# Infant Pertussis Strategy Prioritization With Transmission Blocking and Macrolide Resistance
...
**Main text word count:** 2807

## Key Points

**Question:** Which pertussis strategy characteristics were associated with lower modeled infant burden under vaccine transmission-blocking and macrolide-resistance assumptions?

**Findings:** In this decision analytical model of 10 illustrative country profiles, strategies that reduced infant exposure or assumed stronger transmission-blocking vaccine properties produced larger reductions in modeled infant symptomatic cases than marginal increases in already high childhood vaccination coverage. Resistance-guided management reduced resistant infections under specified implementation assumptions.

**Meaning:** Infant pertussis strategy prioritization should distinguish routine vaccination, pregnancy Tdap, exposure-reduction adjuncts, resistance-sensitive management, and hypothetical vaccine product targets rather than treating them as directly substitutable policies.

## Abstract

**Importance:** Infants remain at highest pertussis risk before completing primary vaccination. Strategy comparisons require separating symptom prevention, transmission blocking, infant exposure, and macrolide-resistant *Bordetella pertussis*.
...
**Results:** Across calibrated profiles, current-practice annual infant case incidence ranged from approximately 16 to 2393 per 100,000 infants; infant incidence was not directly calibrated. Calibration-window mean absolute percentage error for reported cases was 5.5% (interquartile range [IQR], 3.9%-7.8%). Routine childhood coverage floors did not produce a consistent infant-case reduction (median, -1%; IQR, -5% to -1%), and adolescent boosting had a small and uncertain reduction (median, 2%; IQR, 0%-58%). Pregnancy Tdap scale-up produced a median 12% reduction (IQR, 10%-14%); the infant-exposure reduction composite produced a median 45% reduction (IQR, 25%-77%). Targeted high-risk PEP produced a 5% median reduction. Resistance-guided management reduced resistant infections by 97% in the main implementation scenario, with infant-case reductions varying across implementation assumptions. High-transmission-blocking and combined stress-test profiles produced median infant-case reductions of 97% or higher.

**Conclusions and Relevance:** In this decision analytical modeling study, additional modeled infant protection was associated more strongly with infant-exposure reduction, transmission-blocking vaccine properties, and resistance-sensitive management than with marginal childhood coverage increases in already high-coverage profiles. Country-specific estimates should be interpreted as conditional scenario projections rather than national forecasts.
...
The practical decision space is therefore broader than whether to add more doses. Current programmatic discussions distinguish maintaining routine childhood vaccination, improving pregnancy Tdap uptake, protecting infants through close-contact management, using PEP for high-risk infant exposures, and developing vaccines that better reduce colonization or transmission. A decision model can be more useful if it separates near-term program levers, implementation-dependent adjuncts, resistance-sensitive management, and hypothetical product targets.
...
The infant endpoint was nevertheless retained as the primary comparative endpoint because the policy target was prevention of severe disease in children too young to complete primary vaccination. Results therefore emphasize within-profile relative changes rather than absolute national infant counts, and baseline infant incidence, conditional beta-grid intervals, infant-contact sensitivity, maternal-duration sensitivity, and external age-pattern triangulation were reported to make the endpoint's uncertainty visible.
...
The routine childhood coverage scenario raised low routine infant and childhood coverage values to prespecified floors without reducing countries already above those floors. The adolescent booster scenario represented school-age or adolescent booster scale-up while preserving the current aP-like vaccine mechanism. Pregnancy Tdap scale-up increased birth-entry passive protection for infants aged 0 to 2 months. The cocooning adjunct represented improved Tdap-up-to-date status among close adult contacts and conservative adult-to-infant contact reduction. The infant-exposure reduction composite combined pregnancy Tdap scale-up, close-contact protection, and reproductive-age adult boosting. Targeted high-risk PEP increased PEP reach for household contacts, infants, and high-risk infant settings. Resistance-guided management represented testing-informed alternative treatment and restored resistant-strain PEP effectiveness. The combined profile was a stress test of favorable mechanisms, not an implementable package.
...
Strategy profiles showed a separation by decision role (<span style="color:#5DADE2;">Figure 4A-4C</span>). Routine childhood coverage floors did not produce a consistent infant-case reduction (median, -1%; IQR, -5% to -1%), and adolescent booster scale-up produced a small and uncertain reduction (median, 2%; IQR, 0%-58%). This diagnostic evaluated marginal gains above current programs and should not be interpreted as evidence against maintaining or improving timely routine childhood vaccination.

Pregnancy Tdap scale-up produced a median 12% infant-case reduction (IQR, 10%-14%). The close-contact/cocooning adjunct produced a median 37% reduction, and the infant-exposure reduction composite produced a median 45% reduction (IQR, 25%-77%). Component diagnostics produced median reductions of 12% for pregnancy Tdap alone, 35% for reproductive-age adult boosting, and 3% for cocooning/contact reduction alone. Targeted high-risk PEP produced a median 5% reduction. Resistance-guided management reduced resistant infections by 97% under fixed implementation assumptions and produced variable infant-case reductions across implementation sensitivity analyses. The high-transmission-blocking vaccine target and combined stress-test profiles produced median infant-case reductions of 97% or higher.

Country-level heterogeneity was greater for strategies acting through older-age transmission. The childhood coverage floor improved infant outcomes in 1 of 10 profiles and increased infections in most age groups in the age-shift diagnostic. Adolescent boosting produced positive infant-case reductions in 5 of 10 profiles. Pregnancy Tdap scale-up, cocooning adjuncts, infant-exposure reduction, resistance-guided management, high-transmission-blocking vaccine targets, and combined stress-test profiles reduced infant cases in all 10 profiles. For secondary outcomes, infant-exposure reduction reduced median reported cases by 38% and all infections by 35%. Exploratory mortality translations are reported in the Supplement.
...
In near-term treatment-implementation sensitivity analyses, partial resistance-guided uptake with restored PEP produced median infant-case changes ranging from -16% to 16% across uptake assumptions. Scenarios without restored PEP effectiveness produced larger apparent median infant-case reductions in some settings. Resistant infections were reduced under the main resistance-guided assumptions.

### Robustness

In infant-contact sensitivity analyses, increasing child, adolescent, and adult sources into infant target groups from 0.75 to 1.50 changed current-practice 5-year median infant burden from 120 to 186 cases per 100,000 infants; the infant-exposure reduction strategy remained lower across the same multipliers (52 to 78 per 100,000 infants). Varying passive maternal protection from 90 to 270 days changed direct pregnancy Tdap 5-year reductions from 9% to 12% and the infant-exposure reduction strategy from 57% to 59% (<span style="color:#5DADE2;">eTable 17</span>). External age-pattern triangulation showed better agreement for the United States than for England, Sweden/EU/EEA, and Australia.^31-34^ Across analysis windows and selected-parameter sampling, combined and high-transmission-blocking profiles most often had the lowest infant burden (<span style="color:#5DADE2;">eTables 19, 20, and 25</span>).

In ordering diagnostics, the combined stress-test profile had the lowest modeled infant burden in 8 of 10 full-horizon country profiles and in 36 of 50 country-window cells, while the high-transmission-blocking vaccine target was usually among the lowest-burden profiles. Infant-exposure reduction, cocooning adjuncts, pregnancy Tdap scale-up, targeted high-risk PEP, adolescent boosting, and resistance-guided management changed order across countries, infant age strata, and time windows. Because the profile set included available levers, implementation-dependent modifiers, and hypothetical product targets, these orderings were not interpreted as policy rankings.

## Discussion

This decision analytical modeling study reframed pertussis prevention analysis as an infant-protection prioritization problem. Across 10 illustrative profiles, routine childhood vaccination remained foundational, but marginal increases in already high childhood coverage were not the main source of additional modeled infant protection. Larger reductions were associated with strategy characteristics that reduced infant exposure, improved onward-transmission blocking, or modified resistant-strain management. This interpretation aligns with recent evidence that post-pandemic resurgence, waning aP vaccine protection, and incomplete transmission blocking make infant exposure reduction a central policy question.^1,5,6,9-13^

The strategy taxonomy is central to interpretation. Pregnancy Tdap scale-up represented direct passive protection and produced modest, consistent infant benefit, consistent with maternal vaccination effectiveness studies.^26,27^ The infant-exposure reduction composite produced larger reductions, but those reductions reflected adult-boosting and contact assumptions as well as pregnancy Tdap; they should not be described as a maternal-immunization-only effect. Targeted high-risk PEP had smaller average benefit and should be read as outbreak or contact-management support, consistent with guidance that prioritizes household contacts, infants, and high-risk settings rather than broad population control.^30^

This distinction matters for policy communication. Routine childhood vaccination remains the foundation of pertussis control even when marginal coverage-floor simulations show little additional infant benefit. The model asks what incremental strategy characteristics are most informative after current routine programs are represented. It does not imply that countries should divert effort away from timely infant-series completion, catch-up vaccination, or primary-care delivery.
...
The product-target findings should also be interpreted carefully. The high-transmission-blocking profile is not a recommendation for an available vaccine and not evidence that such a product would automatically be programmatically superior once cost, safety, supply, schedule compatibility, and acceptance are considered. Its value is mechanistic: it quantifies how much infant protection could plausibly depend on blocking onward transmission rather than only preventing symptoms.

Macrolide resistance changed management-related outcomes but did not overturn the main strategy-prioritization pattern. Resistance-guided management had simulated benefit under fixed assumptions and was strongly associated with lower resistant infections, but its value depended on testing reach, timely detection, alternative-drug availability, PEP implementation, and resistant-strain fitness. Recent reports of MRBP expansion in China, Japan, Australia, the Americas, and genomic surveillance studies support treating resistance as a modifier of strategy value and uncertainty, not as an independent claim that a specific prevention program should be prioritized over others.^14-21,30^

The resistance findings also imply a surveillance priority. If resistance testing is sparse, slow, or restricted to severe cases, apparent strategy performance may reflect unobserved strain composition as much as vaccination effects. Vaccine strategy assessment in settings with plausible MRBP should therefore be paired with isolate or sequencing surveillance, susceptibility reporting, and explicit assumptions about PEP effectiveness.
...
**Data Sharing Statement:** Processed inputs, model code, configuration files, and generated outputs are publicly available without access restrictions at https://github.com/xmusphlkg/pertussis_transmission_analysis. No individual-level participant data were used or are available.
...
| Resistance-guided management | Dependent on testing reach, turnaround time, alternative treatment, and PEP delivery. | Resistance-sensitive management modifier. | Can reduce resistant infections under specified assumptions, but is not a stand-alone policy priority. |
